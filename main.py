...
class User(UserMixin, db.Model):
    ...
    # Create reference to the BlogPost object, the "author" refers to the author property in the BlogPost class.
    ...
    if form.validate_on_submit():
        if form.validate_on_submit():
            ...
            if User.query.filter_by(email=new_user.email).first():
                db_email = User.query.filter_by(email=new_user.email).first().email
                if db_email == new_user.email:
                    flash("This email already on file.  Please log in instead.")
